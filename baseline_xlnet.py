"""
Baseline: Fine-tuned XLNet
============================
Matches the paper's experimental setup exactly:

  Setup 1 — Binary check-worthiness:
    One XLNet model fine-tuned for binary CW classification.

  Setup 2 — Rationality labels:
    Six INDEPENDENT XLNet models, one per rationality label,
...
Note on XLNet:
  Uses XLNetForSequenceClassification which handles the summary
  token internally — no need for manual last-token pooling.

Usage:
  python baseline_xlnet.py
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "..", "xlnet_baseline_output")
...
print("=" * 60)
print("  Baseline: Fine-tuned XLNet")
print("=" * 60)
...
bin_model = XLNetForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2)
...
for i, col in enumerate(RATIONALITY_COLS):
    ...
    rat_model = XLNetForSequenceClassification.from_pretrained(
